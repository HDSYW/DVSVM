function [result] = SVM_prob(Data,K,ktype,pa)
test_ac=0;
...
for j = pa.min:pa.step:pa.max
    ...
    for power= pa.min:pa.step:pa.max
        ...
        for i = 1:k
            ...
            CM = ConfusionMatrix(PredictY,ValY);M_F(i)=CM.FM;M_GM(i)=CM.GM;
        end
        mean_Acc =mean(M_Acc)                ; mean_Erro=mean(M_Erro);
        mean_Errorate=mean(M_Errorate) ; mean_F=mean(M_F); mean_GM=mean(M_GM);
        if  mean_Acc>test_ac %mean_F>test_F
            test_F=mean_F; test_GM=mean_GM ;
            test_ac=mean_Acc;
            best_Erro=mean_Erro; best_Errorate=mean_Errorate; best_p1=Para.p1; best_kp1=Para.kpar.kp1;
        end
    end
    fprintf('Complete %s\t\n',num2str((j+8)*100/16))
end
% >>>>>>>>>>>>>>>>>>>> Test and prediction <<<<<<<<<<<<<<<<<<<<
TrnX=X_train_T      ; TrnY=Y_train_T;
Para.p1=best_p1; Para.kpar.kp1=best_kp1;
% ---------- Model ----------
[PredictY , model] = LIB_L1SVCprob(X_test_T , Trn , Para);
